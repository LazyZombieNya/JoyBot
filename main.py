...
#BASE_URL = "https://joy.reactor.cc/post/6026893"
...
def parse_post(post):
    ...
    for img_div in post.find_all('div', class_='image'):
        ...
        for video_span in img_div.find_all('span', class_='video_holder'):# если тег image для видео
            ...
            post_data["content"].append({"id":str(uuid.uuid4()),"type": "video", "data": "https:" + video_url, "send": "not"})

        for video_gif_span in img_div.find_all('span', class_='video_gif_holder'):# если тег image для gif
            img_tag = "video_gif_holder"
            img = video_gif_span.find('img')
            if img and img.get('src'):
                img_gif_url = img['src']
                post_data["content"].append({"id":str(uuid.uuid4()),"type": "video", "data": "https:" + img_gif_url, "send": "not"})
        ...
        if img_tag == "": # если тег image для картинок только
            ...
            if img and img.get('src'):
                img_url = img['src']
                post_data["content"].append({"id":str(uuid.uuid4()),"type": "photo", "data": "https:" + img_url, "send": "not"})
        print(f"img_div {img_div}")


    # Работаем с <iframe это coub, youtube, vimeo
    for iframe in post.find_all('iframe'):
        iframe_url = iframe.get('src')
        post_data["content"].append({"id":str(uuid.uuid4()),"type": "video_hosting", "data": iframe_url, "send": "not"})
...
async def send_post(chat_id, post_id, contents,text_content):
    ...
    #    # print(media_content)
    if text_content and (sum(len(text) for text in text_content) + len(title)+ len(link_post)) < LIMIT_CAPTION:
        caption = link_post + title + " ".join(text_content)
        text_content.clear()
    else:
            ...
            if content["type"] == "photo":
                    ...
                    case "err":
                        photo_group.append(
                            InputMediaPhoto(media=content["data"].replace("/full/", "/"),
                                            caption=(caption if not photo_group else None),# убираем /full/ картинка будет не высокого качества
                                            parse_mode="HTML"))  # caption только на первую картинку, так описание к группе будет
                        id_photo.append(content["id"])
                ...
                if content["send"] == "not":
                    keyboard = InlineKeyboardMarkup().add(
                        InlineKeyboardButton("📺 Смотреть видео", url=content["data"])
                    )
